...
async function loadPlayerPage() {
  ...
  const [playerData, names, maps, ratings] = await Promise.all([
    fetch(`data/seasons/${currentSeason}/players/${playerId}.json`).then(r => r.json()),
...
const statsHTML = `
  <div style="font-size:0.78em; line-height:1.85; color:#ccc;">
    <div>Rank: ${rank}</div>
    <div>MMR: ${mmr}</div>
    <div>MMR Uncertainty: ${mmrUncertainty}</div>
    <div>${playerStats.games} Games. ${playerStats.wins} wins. ${losses} losses. (${winrate}%)</div>
  </div>
`;
